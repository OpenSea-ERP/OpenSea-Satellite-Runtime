...
const { autoUpdaterMock, BrowserWindowMock, logMock, electronStoreData } = vi.hoisted(() => {
...
describe('updater module', () => {
  ...
  it('persists pendingUpdateVersion on update-downloaded', () => {
    ...
    expect(electronStoreData.get('updater.preferences')?.pendingUpdateVersion).toBe('2.0.0');
  });
  ...
  it('persists lastFailedUpdateAt and broadcasts error', () => {
    ...
    expect(electronStoreData.get('updater.preferences')?.lastFailedUpdateAt).toBeTypeOf('number');
  ...
  it('suppresses benign releases.atom 404 when opt-in', () => {
    ...
    autoUpdaterMock.emit('error', new Error('HttpError: 404 fetching releases.atom'));
    expect(win.webContents.send).not.toHaveBeenCalled();
    expect(logMock.warn).toHaveBeenCalledWith(expect.stringContaining('feed indisponível'));
  });
  ...
  it('update-downloaded with no announcedRelease does not log cross-check', () => {
    ...
    expect(logMock.warn).not.toHaveBeenCalledWith(expect.stringContaining('NÃO bate'));
    expect(logMock.info).not.toHaveBeenCalledWith(
  ...
  it('cross-checks announced release on update-downloaded (divergent)', () => {
    ...
    expect(logMock.warn).toHaveBeenCalledWith(expect.stringContaining('NÃO bate'));
  });
  ...
  it('cross-check on update-available (divergent)', () => {
    ...
    expect(logMock.warn).toHaveBeenCalledWith(expect.stringContaining('Versão divergente'));
  });
  ...
  it('primeUpdaterStore is idempotent (does not clobber non-default values)', () => {
    ...
    expect(electronStoreData.get('updater.preferences')?.pendingUpdateVersion).toBe('1.0.0');
  });
});
